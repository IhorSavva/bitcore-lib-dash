...
Input.fromBufferReader = function(br, params) {
  ...
  if (params && params.hex) {
    ...
    var sigString = inputList[0].script_sig.slice(4, (parseInt(inputList[0].script_sig.slice(2,4), 16))*2+2)
    var signatures = []
    if(params.publicKey){
      var publicKeysString = params.publicKeys.map(function (key) { return key.toString()})

      signatures[publicKeysString.indexOf(params.publicKey)] = Signature({prevTxId: input.prevTxId, outputIndex: input.outputIndex, inputIndex: 0, sigtype:1, publicKey: params.publicKey, signature: sigString})
    }
    ...
    input = new Input.MultiSigScriptHash(input, params.publicKeys, 2,signatures)

    input.output = new Output(params.utxo)
