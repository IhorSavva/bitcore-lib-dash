 ...
 * @constructor
 */
function Transaction(serialized, params) {
  ...
  if (serialized) {
    if (serialized instanceof Transaction) {
      ...
      this.fromString(serialized, params);
    } else if (BufferUtil.isBuffer(serialized)) {
      this.fromBuffer(serialized, params);
...
Transaction.prototype.fromBuffer = function(buffer, params) {
  var reader = new BufferReader(buffer);
  return this.fromBufferReader(reader, params);
};

Transaction.prototype.fromBufferReader = function(reader, params) {
  ...
  for (i = 0; i < sizeTxIns; i++) {
    var input = Input.fromBufferReader(reader, params);
...
Transaction.prototype.fromString = function(string, params) {
  this.fromBuffer(new buffer.Buffer(string, 'hex'), params);
};
